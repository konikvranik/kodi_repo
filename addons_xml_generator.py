        ...
        for addon in addons:
            try:
                ...
                if (not os.path.isdir(
                    addon) or addon == ".svn" or addon == ".git" or addon == "nbproject" or addon == "zip"): continue
                # create path
                ...
                zversion = re.compile('<addon\s+id=".+?"\s+name=".+?"\s+version="(.+?)"', re.DOTALL).findall(fcontent)
                addonname = re.compile('<addon\s+id="(.+?)"\s+name="', re.DOTALL).findall(fcontent)
                # zip dir
                ...
                copyfile(os.path.join(addon, "changelog.txt"),
                         os.path.join(zaddondir, "changelog-" + zversion[0] + ".txt"))
                # create zipfile
